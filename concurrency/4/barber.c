...
void *barberFn(void *arg){
        while(1){
                ...
                printf("Barber has been woken up\n");
                cut_hair();
        }
}
void *customer(void *number){
        int num = *(int *)number;
        printf("Customer %d is now at the babershop\n", num);

        if(chairs_occupied >= NUM_CHAIRS){
                printf("All chairs occupied. Customer %d is leaving\n", num);
        }
        else{
        /* Wait for chair */
...
void cut_hair(){
        ...
        printf("Barber will take %d seconds to cut hair\n", cut_time);
        sleep(cut_time);
